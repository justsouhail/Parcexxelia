...
<form action="/Materiel/ticket/update/traitement/{{$ticket->id}}" method="post" enctype="multipart/form-data" >
    @csrf
...
<div class="row">
    ...
    <div class="form-group">
      ...
      <select name="Marque" class="form-control  @error('Marque') is-invalid @enderror " id="Marque">
        ...
        @foreach($Marque_tables as $Marque)
            <option value="{{ $Marque->id }}" {{ old('Marque', $ticket->Marque->id ?? null) == $Marque->id ? 'selected' : '' }}>
                {{ $Marque->Marque_Nom }}
            </option>
            @endforeach
      </select>
  ...
  <div class="col-4">
    <div class="form-group" >
      ...
      <select name="Model" class="form-control @error('Model') is-invalid @enderror " id="Model">
        ...
        <option value="{{ $Model->id }}" {{ old('Model', $ticket->Model->id ?? null) == $Model->id ? 'selected' : '' }}>
        @endforeach
      </select>
      @error('Model')
                        <div class="invalid-feedback">{{ $message }}</div>
  ...
  <div class="col-4">
    <div class="form-group" >
      ...
      <input type="text" name="N°_de_serie" class="form-control @error('N°_de_serie') is-invalid @enderror" id="N°_de_serie" placeholder="N°_de_serie" value="{{ $ticket->N°_de_serie}}" >
      @error('N°_de_serie')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
    </div>
...
<div class="col-4">
    <div class="form-group ">
        ...
        <input type="text" name="Addresse_IP" class="form-control @error('Addresse_IP') is-invalid @enderror" id="Addresse_IP" placeholder="Addresse_IP" value="{{ $ticket->Addresse_IP}}" >
        @error('Addresse_IP')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
